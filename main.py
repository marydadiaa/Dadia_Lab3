...
def get_detailed_posts(userID: int):
    # Retrieve all posts for the specific user
    user_posts = get_post_then_format_according_to_user(userID=userID)
    ...
    for post in user_posts["posts"]:
        # Fetch comments for the current post by calling the get_comments function
        post_comments = requests.get(f"http://127.0.0.1:8000/comments/?postId={post['id']}")
        comments_data = json.loads(post_comments.text)
        
        # Format the post and its comments
